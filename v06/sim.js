import {VERSION,SAVE_KEY,PACKING,PARTS,ROADS,node,random,siteItems} from './world.js?v=0.6.0-human-nature-1';
import {startFishing,fishingAction,updateFishing} from './fishing.js?v=0.6.0-human-nature-1';
import {startSalvage,salvageAction,updateSalvage,saveSite} from './salvage.js?v=0.6.0-human-nature-1';
import {showStory,roadShots,updateReel} from './journey.js?v=0.6.0-human-nature-1';
import {clamp,log,slots,consumeFresh,spend,finish,hurt,shopStock,OFFERS} from './state.js?v=0.6.0-human-nature-1';
import {makeIncident,resolveIncident} from './conflict.js?v=0.6.0-human-nature-1';
import {advanceRoad} from './drive.js?v=0.6.0-human-nature-1';
...
export function endActivity(s){
 ...
 if(a.type==='repair'){
  ...
  if(a.phase==='done'){
   ...
   if(a.target==='pump'){s.sponsor=true;s.flags.pumpDone=true;s.meals+=2;s.fuel=Math.min(50,s.fuel+4);s.cash+=25;log(s,'Water running',"Glenn Holt signed for the Mercer household. Your place in Morrow now has a witness. The settlement shares two meals, four fuel, and $25.");}
...
export function act(s,a){
 ...
 case 'delivery':if(s.mode!=='stop'||!['freight','ridge'].includes(s.node))throw Error('Find the Pruitt convoy.');if(s.cargo)throw Error('The filters are already aboard.');if(slots(s)>4)throw Error('The crate needs three locker spaces. Fit or trade parts first.');s.cargo={condition:100};s.flags.tarp=true;spend(s,1.5);log(s,'A place on the manifest','Wes Pruitt put the Mercers on the delivery list. The crate is covered and tied down. Get the filters to Morrow intact.');break;
...
export function update(s,dt){
 ...
 if(s.mode==='travel'){
  ...
  if(r.elapsed>=r.duration){s.node=r.to;s.visited.push(r.to);s.stats.miles+=Math.round(r.hours*35);s.condition=Math.max(0,s.condition-r.wear);s.energy=clamp(s.energy-r.hours*2.4,0,100);if(s.energy<15)s.health=Math.max(1,s.health-7);if(s.cargo&&r.kind==='storm')s.cargo.condition=clamp(s.cargo.condition-(s.flags.tarp?3:12));s.mode='stop';s.road=null;if(s.condition<=0){log(s,'The van gives out at the stop','You coast into the yard. Repair the running gear before driving again.');return;}if(s.mode==='stop')log(s,node(s.node).name,node(s.node).desc);}
 }
