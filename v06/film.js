import {drawRoad,drawVan} from './motion.js?v=0.6.0-signals-end-02';
...
export function filmFrame(c,w,h,id,t,images){
 c.clearRect(0,0,w,h);
 if(id==='highway'){
  c.save();c.translate(Math.sin(t*7)*.25,Math.sin(t*3)*.35);cover(c,images.windshield,w,h,1.015+t*.022,.5,.5);c.restore();
  ...
  cover(c,images.plaza,w,h,1.06,.48+t*.003,.5);c.fillStyle='#102c3530';c.fillRect(0,0,w,h);c.strokeStyle='#cedee755';c.lineWidth=1;
  ...
  cover(c,images.scanner,w,h,1.03,.5,.5);const x=w*(.16+t*.24),beam=c.createLinearGradient(x-100,0,x+100,0);beam.addColorStop(0,'#def6ff00');beam.addColorStop(.5,'#e6f8ff55');beam.addColorStop(1,'#def6ff00');c.fillStyle=beam;c.fillRect(0,0,w,h);c.fillStyle='#bde3e311';c.fillRect(0,h*.50+Math.sin(t*.7)*h*.12,w,3);
 }else{
  cover(c,images.blocked,w,h,1.05,.49+t*.002,.5);c.save();c.translate(w*1.02,h*.57);c.rotate(-.6+Math.min(1,t/2.5)*.6);const metal=c.createLinearGradient(0,-h*.025,0,h*.025);metal.addColorStop(0,'#d6c7a1');metal.addColorStop(.35,'#9f9d83');metal.addColorStop(1,'#383f38');c.fillStyle=metal;c.fillRect(-w*.82,-h*.025,w*.82,h*.05);c.save();c.beginPath();c.rect(-w*.82,-h*.025,w*.82,h*.05);c.clip();c.strokeStyle='#995841';c.lineWidth=w*.03;for(let x=-w;x<0;x+=w*.09){c.beginPath();c.moveTo(x,-h*.04);c.lineTo(x+w*.05,h*.04);c.stroke();}c.restore();c.strokeStyle='#26332f';c.lineWidth=2;c.strokeRect(-w*.82,-h*.025,w*.82,h*.05);for(let i=0;i<260;i++){const x=-w*.82+mod(i*63.17,w*.82),y=-h*.023+mod(i*7.71,h*.046);c.fillStyle=i%2?'#3d473860':'#d3c4a450';c.fillRect(x,y,2+mod(i,4),1);}for(let x=-w*.78;x<0;x+=w*.11){c.fillStyle='#27332d';c.beginPath();c.arc(x,0,2.3,0,7);c.fill();}c.restore();const light=c.createRadialGradient(w*.86,h*.57,0,w*.86,h*.57,75);light.addColorStop(0,'#ffd68655');light.addColorStop(1,'#ffd68600');c.fillStyle=light;c.fillRect(0,0,w,h);
 }
