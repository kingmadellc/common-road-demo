import {VERSION,SAVE_KEY,FAMILY,NODES,ROADS,PACKING,PARTS,node,roads} from './world.js?v=0.5.0';
import {fresh,load,save,act,update,summary,slots,clamp} from './sim.js?v=0.5.0';
import {render,ready,HAZARDS,HOME_POINTS,sources} from './art.js?v=0.5.0';
import {castEndpoint} from './fishing.js?v=0.5.0';
import {yardView} from './scenes.js?v=0.5.0';
import {SITE_NAMES} from './salvage.js?v=0.5.0';
import {collectorOptions,incidentOptions,threatLabel} from './conflict.js?v=0.5.0';
import {hurt,shopStock,OFFERS} from './state.js?v=0.5.0';
import {SHOTS} from './journey.js?v=0.5.0';
import {enableAudio,soundFrame} from './sound.js?v=0.5.0';
...
function drawUI(force=false){
 ...
 if(ui.title){const key='title'+!!saved;if(force||panelKey!==key){$('opening').innerHTML=`<div class="label">A SURVIVAL PILGRIMAGE</div><h1>Somewhere,<br><em>still ours.</em></h1><p>A family. A van we actually own. Two days to a place beyond Continuum’s reach — where a home and a life can still be yours.</p><div class="buttons">${saved?button('resume',undefined,'Continue the journey','','primary'):''}${button('new',undefined,saved?'Start fresh':'Play Common Road','',saved?'':'primary','id="start"')}</div><div class="legacy">THE LINE · PLAYABLE CHAPTER 05<br><a href="v04/">Continue your v0.4 journey</a></div>`;panelKey=key;}return;}
...
function perform(type,id,target){
 if(type==='new'){if(saved)modal('new',`<h2>Start another pilgrimage?</h2><p>This replaces the v0.5 journey saved in this browser. Your previous v0.4 journey stays separate.</p>${button('confirmNew',undefined,'Start fresh','','danger')}`);else startNew();return;}
