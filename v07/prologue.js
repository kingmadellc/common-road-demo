import {FILM_ID,FILM_DURATION,FILM_SCENES,filmCaption,filmSceneAt,drawOpeningFrame} from './opening-film.js?v=0.8.5-safety-1';
export const INTRO_KEY='signals-end-opening-history';
export const BEATS=FILM_SCENES;
export function readOpeningHistory(storage){try{const value=JSON.parse(storage.getItem(INTRO_KEY));return [FILM_ID,'run-for-it-6','run-for-it-5','run-for-it-4','run-for-it-3','run-for-it-2','run-for-it-1'].includes(value?.id)&&['started','skipped','watched'].includes(value.status)?value:null;}catch{return null;}}
