import {FILM_ID,FILM_DURATION,FILM_SCENES,filmSceneAt,drawOpeningFrame} from './opening-film.js?v=0.7.2-canon-1';
...
export function rememberOpening(storage,status){try{const old=readOpeningHistory(storage),rank={started:1,skipped:2,watched:3};if(old&&rank[old.status]>rank[status])return;storage.setItem(INTRO_KEY,JSON.stringify({id:FILM_ID,status,at:Date.now()}));}catch{}}
...
export function prologueState(p){return p?{id:p.id,time:Math.round(p.time*10)/10,duration:FILM_DURATION,index:p.index,paused:p.paused,muted:p.muted,source:p.source,blocked:p.blocked,caption:filmSceneAt(p.time).line,replay:p.replay}:null;}
...
export {FILM_ID,FILM_DURATION,FILM_SCENES};
