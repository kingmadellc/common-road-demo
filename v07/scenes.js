import {WATER_ART,SITE_ART} from './media.js?v=0.8.9-sequence-1';
import {drawSceneMotion,drawSearchLight,drawRecoveredPart} from './scene-motion.js?v=0.8.9-sequence-1';
import {YARD,layoutFor} from './salvage.js?v=0.8.9-sequence-1';
import {FISH_PROFILES,castEndpoint,fishingBounds} from './fishing.js?v=0.8.9-sequence-1';
...
export function drawSalvage(c,w,h,s,ui,images){
 ...
 const art=SITE_ART[s.node],im=images[art];imageCover(c,im,0,0,w,h);drawSceneMotion(c,w,h,s,art,images);
 if(g.player.hiding){c.fillStyle='#081719b5';c.fillRect(0,0,w,h);}
 drawSearchLight(c,w,h,s);
 ...
 if(g.phase==='intercept'){const im=images.collectorSprite;if(im?.naturalWidth){const ih=h*.65,iw=ih*im.naturalWidth/im.naturalHeight;c.drawImage(im,w*.5-iw/2,h*.32,iw,ih);}}else if(item&&!g.player.hiding)drawRecoveredPart(c,w,h,item,s);
...
export function drawFishing(c,w,h,s,ui,images){
 const g=s.activity,art=WATER_ART[s.node]||'fishing';imageCover(c,images[art],0,0,w,h);drawSceneMotion(c,w,h,s,art,images);
 ...
 for(const f of g.fish){if(f.caught||['landing','landed'].includes(g.phase)&&f.id===g.target)continue;const active=f.id===g.target&&['fight','landing','landed'].includes(g.phase),x=f.x*w,y=f.y*h,angle=active?g.runDir*.2:Math.sin(g.elapsed*.45+f.phase)*.22;
 ...
 if(g.phase==='aim'){
  let target=g.aim;if(ui.cast&&s.settings.castMode==='pull')target=castEndpoint(ui.cast,{x:ui.cast.lastX,y:ui.cast.lastY},fishingBounds(s));
 ...
 if(['landing','landed'].includes(g.phase)){
  ...
  const fish=g.fish.find(f=>f.id===g.target),length=fish?.kind==='heavy'?.078:fish?.kind==='dart'?.065:.052;c.fillStyle=fish?.kind==='heavy'?'#9c9e81':fish?.kind==='dart'?'#a3b7b1':'#b4bb99';c.beginPath();c.ellipse(nx,ny,w*length,h*.018,-.2,0,7);c.fill();c.fillStyle='#b77c50';c.beginPath();c.moveTo(nx-w*.072,ny);c.lineTo(nx-w*.1,ny-h*.02);c.lineTo(nx-w*.098,ny+h*.027);c.fill();
 }
}
