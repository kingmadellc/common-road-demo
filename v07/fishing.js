import {random} from './world.js?v=0.8.9-sequence-1';
...
export const fishingBounds=s=>s.node==='river'?{minX:.10,minY:.27}:{minX:.24,minY:.45};
export function castEndpoint(start,end,bounds={minX:.10,minY:.27}){
 const dx=clamp(start.x-end.x,-.46,.46),pull=clamp(end.y-start.y,.0,.36);
 return {x:clamp(.5+dx*1.4,bounds.minX,.88),y:clamp(.76-pull*2.1,bounds.minY,.76),valid:Math.hypot(end.x-start.x,end.y-start.y)>.035};
}
export function startFishing(s){
 s.fishSites??={};
 if(!s.fishSites[s.node]){
  const r=random(s.seed+s.node.length*91);
  s.fishSites[s.node]=['small','small','dart','heavy'].map((kind,id)=>({id,kind,x:[.3,.65,.42,.76][id],y:Math.max(fishingBounds(s).minY+.015,[.61,.64,.50,.33][id]),phase:r()*6.28,caught:false,spooked:0}));
 }
...
export function fishingAction(s,a){
 ...
 if(a.type==='cast'){
  ...
  g.hook={x:clamp(a.x,fishingBounds(s).minX,.88),y:clamp(a.y,fishingBounds(s).minY,.76)};g.casts++;g.castMode=a.method||s.settings.castMode;g.target=null;g.progress=0;
  ...
  setPhase(g,'cast','');
 }else if(a.type==='fishAim'){g.aim={x:clamp(a.x,fishingBounds(s).minX,.88),y:clamp(a.y,fishingBounds(s).minY,.76)};}
...
export function updateFishing(s,dt){
 const g=s.activity,bounds=fishingBounds(s);g.hook.y=clamp(g.hook.y,bounds.minY,.76);g.elapsed+=dt;g.phaseTime+=dt;
 for(const f of g.fish){f.x=clamp(f.x,bounds.minX,.88);f.y=clamp(f.y,bounds.minY,.8);f.spooked=Math.max(0,f.spooked-dt);if(f.caught||(['fight','landing','landed'].includes(g.phase)&&f.id===g.target))continue;
  if(f.id===g.target&&['wait','bite'].includes(g.phase)){f.x+=(g.hook.x-f.x)*dt*.65;f.y+=(g.hook.y-f.y)*dt*.65;}
  else{f.x=clamp(f.x+Math.sin(g.elapsed*.45+f.phase)*dt*.010,bounds.minX,.88);f.y=clamp(f.y+Math.cos(g.elapsed*.32+f.phase)*dt*.003,bounds.minY,.70);}}
 ...
 if(g.phase==='fight'){
  ...
  f.y=clamp(.79-g.distance*(.79-bounds.minY)/.94,bounds.minY,.80);g.progress=clamp(1-g.distance/.78);
