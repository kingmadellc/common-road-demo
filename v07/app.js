...
function panel(){
 ...
 if(s.mode==='ending')return group(s.ending.title,`<p>${s.ending.text}</p>${s.ending.owned?`<div class="deed"><div class="label">SIGNALS END CHARTER · RECORDED OWNERSHIP</div><h3>JACK & SARAH MERCER</h3><p>One home. A garden. A place to work with your hands.<br>No rent. No employer tied to your door. Phones, computers and digital devices stayed at the Line. Local dues are public, voted on, and capped by the charter.</p><p>${s.packed.includes('chair')?'Grandad’s chair':'Your armchair'} sits ${s.home.spot==='window'?'beside the window':'by the workbench'}. Ben wants to learn the pump. Annie and Rusty are choosing a room.</p></div>`:''}<p>${s.stats.salvaged} items recovered · ${s.stats.fish} fish landed · ${s.stats.repairs} repairs completed · ${count(s.hour)} hours</p><div class="buttons">${button('newConfirm',undefined,'Another pilgrimage','Try the other road.','primary')}${button('journal',undefined,'Read the journey')}</div>`,true);
 return '';
}
...
function showMap(){
 ...
 modal('map',`<div class="label">${complete?'THE MERCERS MADE IT':'DAY '+(Math.floor(s.hour/24)+1)+' · CHAPTER '+current.chapter+' OF 8'}</div><h2>${complete?'Home, at Signals End.':'Go east. Get your life back.'}</h2><p class="journey-subtitle">San Francisco → the Missouri–Arkansas Ozarks</p>${map}<section class="journey-next"><span class="label">${s.road?'ON THE ROAD TO '+node(s.road.to).short.toUpperCase():'YOU ARE HERE · '+current.short.toUpperCase()}</span><h3>${objective(s)}</h3><p>${admission(s)}</p>${!['ending','home'].includes(s.mode)?`<p class="convoy-clock">${count(s.deadline-s.hour)} hours until the guarded convoy leaves. Miss it and this reservation goes to the next family.</p>`:''}${s.mode==='stop'&&s.node!=='line'?button('chooseRoad',undefined,s.node==='plains'?'Choose how to earn your way in':'Plan the next drive →','','primary'):s.node==='line'&&s.mode==='stop'?button('reviewCharter',undefined,'Review entry & cross','','primary'):''}</section><details class="whole-trail"><summary>The whole trail · ${s.stats.miles.toLocaleString()} miles traveled</summary><ol class="chapter-list">${chapters.map(([name,place,ids],i)=>{const here=ids.includes(s.node),done=ids.some(id=>s.visited.includes(id))&&!here;return `<li class="${here?'current':done?'done':''}" ${here?'aria-current="step"':''}><span>${done?'✓':String(i+1).padStart(2,'0')}</span><div><strong>${name}</strong><p>${place}${here?' · You are here':''}</p></div></li>`;}).join('')}</ol></details><div class="buttons">${button('world',undefined,'Why Signals End?','The promise, the rules and the people keeping it alive.')}</div>`);
}
...
function perform(type,id,target){
 ...
 if(type==='reviewCharter'){modal('charter',`<div class="label">EVELYN WARD · THE BORDER STOREHOUSE</div><h2>A choice you make freely.</h2><img class="lore-image" src="assets/eastbound-07/signals-end-basin.webp" alt="The reservoir refuge beyond the guarded crossing"><p>${admission(s)}.</p><ul><li>Phones, computers and all digital devices stay in a numbered box outside the Line. Even Ben’s dead handheld. They can be reclaimed if you leave.</li><li>The same ban applies to the council. Mechanical engines, electricity and analog radios are allowed.</li><li>Your deed is independent of any job. Shared dues are public, voted on and capped. You may leave freely.</li></ul><p>Ben turns the handheld over once, then puts it down. “Can I build something better?” Jack hands him the wrench.</p><div class="buttons">${button('gate',undefined,'Store the devices & cross','Accept the charter. Bring everyone across.','primary',s.sponsor||s.cargo?.condition>=45?'':'disabled')}${button('gateHelp',undefined,'Other ways forward')}</div>`);return;}
 if(type==='world'){modal('world',`<div class="label">FRANK’S LETTER · WHY WE ARE GOING</div><h2>The company stops at the gate.</h2><img class="lore-image" src="assets/eastbound-07/signals-end-basin.webp" alt="Fictional reservoir towns in the Ozarks"><p><strong>Signals End is a place.</strong> A fictional refuge in the Missouri–Arkansas Ozarks. Reservoir crews, farmers and town clerks preserved water, power and land records when services failed. Their quiet provisional council refused Continuum’s takeover. Outsiders call it a shadow government; inside, it has a charter and elections.</p><p><strong>The Line is a border.</strong> Guarded bridges, water patrols and watched access roads protect a small inhabited basin. The hills are no force field. Collectors have enforcement contracts outside; a costly invasion of the basin would threaten the water and power their own customers need.</p><p><strong>The name is a promise.</strong> Account commands stop here. Every digital device stays outside, including offline computers and the council’s equipment. Electricity, mechanical machines and analog radio still work. Signals do not magically disappear. The rule is a deliberate choice, and the same for everyone.</p><p><strong>Frank is Sarah’s uncle.</strong> He got out before them. His letter holds one vacant house for this convoy. The road contacts check who the Mercers are. Glenn’s signed witness or the Pruitts’ ceramic filters activates their reservation. Refuge staff match admissions to water, food and vacant homes; families are admitted together.</p><p><strong>Ownership is the reward.</strong> The homestead fund buys from willing sellers and settled estates. Frank reserved one of those homes. The deed survives losing a job. Local dues are public, voted on and capped. Leaders get no digital exception. People can leave and reclaim their stored devices. It is freedom with chores, not a flawless paradise.</p><p>The toilet, regrettably, is still your problem.</p>`);return;}
