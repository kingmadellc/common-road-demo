import {startDeparture,departureAction,updateDeparture,departureScene,departureSummary,DECISION_INDEX,NOTICE} from './departure.js?v=0.8.2-account-1';
import {freshCapacity} from './hunting.js?v=0.8.2-account-1';
import {CORPORATIONS} from './institutions.js?v=0.8.2-account-1';
import {BRAND} from './brand.js?v=0.8.2-account-1';
import {INTRO_KEY,newPrologue,prologueState,readOpeningHistory,rememberOpening,shouldAutoOpen} from './prologue.js?v=0.8.2-account-1';
import {OpeningPlayer} from './opening-player.js?v=0.8.2-account-1';
import {VERSION,SAVE_KEY,FAMILY,NODES,ROADS,PACKING,PARTS,CAMPAIGN,objective,admission,node,roads} from './world.js?v=0.8.2-account-1';
import {fresh,load,save,act,update,summary,slots,clamp} from './sim.js?v=0.8.2-account-1';
import {render,ready,HAZARDS,HOME_POINTS,sources} from './art.js?v=0.8.2-account-1';
import {castEndpoint} from './fishing.js?v=0.8.2-account-1';
import {yardView} from './scenes.js?v=0.8.2-account-1';
import {SITE_NAMES} from './salvage.js?v=0.8.2-account-1';
import {collectorOptions,incidentOptions,threatLabel} from './conflict.js?v=0.8.2-account-1';
import {hurt,shopStock,OFFERS} from './state.js?v=0.8.2-account-1';
import {SHOTS} from './journey.js?v=0.8.2-account-1';
import {enableAudio,soundFrame} from './sound.js?v=0.8.2-account-1';
import {knowledge,ORIGIN,RADIO_MESSAGE,arriveNarrative} from './narrative.js?v=0.8.2-account-1';
const $=id=>document.getElementById(id),canvas=$('scene');
const departurePreview=new URL(location.href).searchParams.get('departure')==='1';
const huntingPreview=new URL(location.href).searchParams.get('hunt')==='1';
...
if(huntingPreview){s=fresh(71);s.mode='stop';s.node='river';s.hour=12;s.fan=true;s.meals=1.5;s.energy=70;s.threat.identified=true;s.visited=['yard','river'];arriveNarrative(s,'yard');arriveNarrative(s,'river');act(s,{type:'activity',id:'hunting'});ui.title=false;}
if(departurePreview){s=fresh(71);startDeparture(s);ui.title=false;}
let oldSave=null,openingPlayer=null;try{oldSave=localStorage.getItem('common-road-pilgrimage-v6');}catch{}
if((!saved||departurePreview)&&matchMedia('(prefers-reduced-motion: reduce)').matches)s.settings.reducedMotion=true;
...
function finishPrologue(status='skipped'){const p=ui.prologue;if(!p)return;recordOpening(status);openingPlayer?.destroy();openingPlayer=null;ui.prologue=null;ui.title=p.returnTitle;ui.paused=false;if(!p.replay&&status==='watched'){finishNew();return;}drawUI(true);requestAnimationFrame(()=>{const target=s.mode==='departure'&&!ui.title?$('opening').querySelector('[data-action="depNext"]'):ui.title?$('start'):$('scene');target?.focus({preventScroll:true});});}
...
function dispatch(action){try{const beforeMode=s.mode,beforePhase=s.activity?.phase;const next=structuredClone(s);act(next,action);s=next;if(!['reel','tune','steer','slow','fishAim','huntAim','huntViewport'].includes(action.type))persist();drawUI();if(s.mode==='hunting'&&beforePhase!==s.activity.phase&&!ui.paused)$('actions').querySelector('button:not(:disabled)')?.focus({preventScroll:true});if(beforeMode!==s.mode&&!ui.paused)$('stage').scrollIntoView({block:'start',behavior:'instant'});}catch(e){s.message=e.message;$('notice').textContent=e.message;}}
function startNew(){finishNew();}
function finishNew(){if(huntingPreview){location.href='./?v=0.8.0';return;}const settings={...s.settings};resetInput();s=fresh(Math.floor(Math.random()*100000));s.settings=settings;startDeparture(s);ui.title=false;ui.dialog=null;ui.paused=false;if($('dialog').open)$('dialog').close();persist();drawUI(true);}
function resume(){if(huntingPreview){location.href='./?v=0.8.0';return;}s=structuredClone(saved||s);resetInput();ui.title=false;drawUI(true);}
...
function panel(){
 ...
 if(s.mode==='packing')return group(`Six spaces. What comes with you? <span class="pill">${packCount()} / 6</span>`,`<p>Tools, a fishing rod, blankets and Ben’s written broadcast are aboard. They found $240 in the parts tin. Six spaces left.</p><div class="packing">${PACKING.map(i=>button('pack',i.id,`${i.icon} ${i.name} · ${i.slots}`,i.text,s.packed.includes(i.id)?'selected':'',`aria-pressed="${s.packed.includes(i.id)}"`)).join('')}</div>`,true)+group('Bea first.',`<p>Core fired Jack. Index froze their joint account. Their staff flat expires tomorrow. Sarah still wants answers, but staying is a gamble too. Bea is across the Bay.</p>`,false)+group('Keep one another going',`<div class="buttons">${button('depart',undefined,'Go ask Bea','Cross the Bay. Ask your questions. Fix the fan.','primary')}</div>`);
 if(s.mode==='stop'){
  let main;
  ...
  if(n.id==='river')main=button('food',undefined,'Find food',`${count(s.meals*8)} hours left · hunt, fish or trade`,'primary');
 ...
 if(s.mode==='hunting'){
  if(g.phase==='brief')return group('June’s offer',`<p>Plenty only takes digital payment. Your Index account is locked. June lends a rifle and three rounds if Jack fixes her stove.</p><div class="buttons">${button('huntOutfit',undefined,'Fix the stove · borrow the rifle','1 hour · a lesson in the clearing','primary')}${button('huntLeave',undefined,'Try another way')}</div>`,true);
  if(g.phase==='field')return group('Make it count',`<p class="activity-copy">${g.notice}</p><div class="buttons">${button('huntLeave',undefined,'Lower rifle · leave')}${button('huntSound',undefined,s.settings.sound?'Sound on':'Sound off')}</div>`,true);
  if(g.phase==='harvest')return group(`${g.pending} portions to divide`, `<p>Four portions feed everyone. Fresh space: ${Math.floor(freshCapacity(s)*4)} portions. ${s.packed.includes('cooler')?'Ice lasts 24 hours.':'No cooler: use fresh food within 6 hours.'}</p><div class="buttons harvest-actions">${!g.cooked&&g.pending>=4?button('huntFood','cook','Cook supper','4 portions · ½ hour · restores family','primary'):''}${button('huntFood','pack','Pack fresh',`${Math.min(g.pending,Math.floor(freshCapacity(s)*4))} portions fit`,'',freshCapacity(s)<.25?'disabled':'')}${button('huntFood','preserve','Smoke the rest','2 hours · food keeps on the road')}${button('huntFood','share','Share the rest',g.pending>=4&&!s.hunting.shared?'June’s contact helps later.':'Feed the other travelers.')}</div>`,true);
  return group(g.harvested?'Enough for the road':'Back before dark',`<p>${g.notice}</p><p>${g.harvested?`${g.cooked?'Supper cooked · ':''}${g.kept} fresh · ${g.preserved} preserved · ${g.shared} shared`:`${s.hunting.ammo} rounds left. Fish or trade if you need food.`}</p><div class="buttons">${button('huntLeave',undefined,'Back to the family','','primary')}${huntingPreview?button('huntPreviewAgain',undefined,'Try the hunt again'):''}</div>`,true);
 }
 if(s.mode==='fishing'){
  let actions='';
...
function drawDepartureUI(force=false){
 const p=s.departure,shot=departureScene(s),reduced=s.settings.reducedMotion,key='departure:'+p.index+':'+ui.paused+':'+reduced;
 if(!force&&key===panelKey)return;
 const active=document.activeElement?.dataset?.action;
 const decision=p.index===DECISION_INDEX;
 $('opening').innerHTML=`<section class="departure-stage ${shot.notice||shot.termination?'has-notice':''}" aria-label="The night we left"><div class="departure-top">${button('depPause',undefined,ui.paused?'Resume':'Pause')}${!decision?button('depSkip',undefined,shot.loading?'Skip loading':'Skip to decision'):''}</div>${shot.termination?`<aside class="departure-notice termination-notice"><strong>CORE</strong><p class="termination-name">Jack Mercer · Maintenance</p><h2>Employment terminated</h2><dl><dt>Reason recorded</dt><dd>Refusal to certify equipment</dd><dt>Site access</dt><dd>Revoked · effective immediately</dd></dl></aside>`:''}${shot.notice?`<aside class="departure-notice"><strong>INDEX</strong><h2>Payments blocked</h2><dl><dt>Account standing</dt><dd>Restricted · adverse Core report</dd><dt>Joint wallet</dt><dd>Funds frozen pending review</dd><dt>Forma staff flat</dt><dd>Return keys by 06:00 tomorrow</dd></dl></aside>`:''}<div class="departure-copy" aria-live="polite"><p class="departure-kicker">${shot.kicker}</p>${shot.speaker?`<p class="departure-speaker">${shot.speaker}</p>`:''}<p class="departure-line">${shot.termination?'':shot.line}</p></div><div class="departure-actions">${decision?button('depGo',undefined,'Bea first. Get everyone in.','','primary'):button('depNext',undefined,shot.loading?'Next cut →':'Next →','','secondary')}${decision||shot.notice?button('depNotice',undefined,'Read the notice','','secondary'):''}</div>${departurePreview?'<p class="departure-footnote">Departure preview · your journey save is untouched</p>':''}</section>`;
 panelKey=key;
 const wanted=decision?'depGo':active||'depNext';$('opening').querySelector(`[data-action="${wanted}"]`)?.focus({preventScroll:true});
}
function doDeparture(type){
 if(s.mode!=='departure')return;
 if(type==='depPause'){ui.paused=!ui.paused;drawUI(true);return;}
 if(type==='depNotice'){modal('departureNotice',`<div class="label">${NOTICE.issuer} · MERCER HOUSEHOLD</div><h2>${NOTICE.title}</h2><p>${NOTICE.detail}</p><dl>${NOTICE.items.map(([label,value])=>`<dt><strong>${label}</strong></dt><dd>${value}</dd>`).join('')}</dl>`);return;}
 if(ui.dialog)return;
 ui.paused=false;departureAction(s,{depGo:'go',depSkip:'skip',depNext:'next'}[type]);persist();drawUI(true);
...
function drawUI(force=false){
 ...
 const g=s.activity;const key=JSON.stringify([s.mode,s.node,g?.phase,g?.selected,g?.connections,g?.notice,g?.catch,g?.working?.id,g?.pending,g?.cooked,g?.kept,g?.preserved,g?.shared,!!g?.player?.target,g?.player?.hiding,g?.patrol?.state,s.road?.shot?.id,s.story?.id,s.settings,s.packed,s.parts,s.cargo,s.sponsor,s.home,s.flags,s.cash,Math.floor(s.hour),s.threat.permitUsed,s.threat.heat,ui.wire?.index]);
 if(force||key!==panelKey){const active=document.activeElement;const token=active?.dataset?.action?[active.dataset.action,active.dataset.id]:null;const id=active?.id;$('actions').innerHTML=panel();panelKey=key;if(token){const el=[...$('actions').querySelectorAll('button')].find(x=>x.dataset.action===token[0]&&x.dataset.id===token[1]);(el||(s.mode==='hunting'?$('actions').querySelector('button:not(:disabled)'):null))?.focus({preventScroll:true});}else if(id&&$(id)&&document.activeElement===document.body)$(id).focus({preventScroll:true});}
 const notice=s.mode==='hunting'||s.mode==='ending'||s.mode==='stop'&&s.message===node(s.node).desc?'':s.message;if($('notice').textContent!==notice)$('notice').textContent=notice;
 const title=s.mode==='story'?SHOTS[s.story.id].title:s.mode==='packing'?'Everything we can carry':s.mode==='salvage'?SITE_NAMES[s.node]:s.mode==='repair'?(g.target==='pump'?'Bring the water back':'A machine worth saving'):s.mode==='hunting'?'June’s clearing':s.mode==='fishing'?'Dinner is out there':s.mode==='crossing'?'The last crossing':s.mode==='home'?'An evening of our own':s.mode==='ending'?s.ending.title:s.mode==='travel'||s.mode==='incident'?s.road.name:node(s.node).name;
 const region=departurePreview?'DEPARTURE PREVIEW · SAVE UNTOUCHED':huntingPreview?'HUNTING PREVIEW · SAVE UNTOUCHED':s.mode==='home'||s.ending?.owned?'SIGNALS END · THE FIRST EVENING':s.mode==='packing'?'THE MERCER HOUSEHOLD':node(s.node).region;
 ...
 if(s.mode==='hunting'){status=g.phase==='field'?`${s.hunting.ammo} ROUNDS · ${Math.ceil(g.left)}s OF LIGHT${g.warning?' · PATROL IN '+Math.ceil(g.warning)+'s':g.patrol?' · PATROL ON HIGHWAY':''}`:'';hint=g.phase==='field'?'DRAG TO AIM · HOLD STILL · RELEASE TO SHOOT':'';}
...
function showFood(){modal('food',`<h2>Dinner without a login.</h2><p>Plenty takes authenticated digital payments only. The independent camp takes cash, parts and work.</p><div class="buttons">${button('activity','hunting','Hunt the clearing',s.hunting.outfitted?s.hunting.ammo+' rounds left':'Help June · borrow a rifle','primary',s.hunting.outfitted&&(!s.hunting.ammo||s.hunting.taken.length>=2)?'disabled':'')}${button('activity','fishing','Fish the river','Keep what you can carry')}${button('shop',undefined,'Trade at camp',`$${s.cash} · limited stock`)}</div>`);}
...
function sceneDown(p){if(ui.title||ui.paused)return;const h=at(p),g=s.activity;
 ...
 if(s.mode==='hunting'&&g.phase==='field'){
  const aim=p.y>.75?{...g.aim}:{x:p.x,y:p.y-(p.pointerType==='touch'?.12:0)};ui.hunt={start:p,aim};dispatch({type:'huntAim',...aim,value:true});
 }
 if(s.mode==='fishing'){
  if(g.phase==='aim'){
...
function sceneMove(p){if(ui.title||ui.paused)return;
 if(ui.hunt&&s.mode==='hunting')dispatch({type:'huntAim',x:ui.hunt.aim.x+(p.x-ui.hunt.start.x),y:ui.hunt.aim.y+(p.y-ui.hunt.start.y)});
...
function sceneUp(p,cancel=false){
 if(ui.hunt&&s.mode==='hunting'){if(cancel||ui.paused)dispatch({type:'huntAim',value:false});else dispatch({type:'huntFire'});ui.hunt=null;}
...
document.addEventListener('keydown',e=>{
 ...
 if(s.mode==='departure'&&!ui.title&&!ui.dialog){if(e.key==='Escape'||e.code==='Space'){e.preventDefault();doDeparture('depPause');}else if(e.key==='ArrowRight'){e.preventDefault();doDeparture('depNext');}return;}
...
function gamepad(dt){
 ...
 if(!pad){if(prevButtons.length&&s.mode==='hunting'){s.activity.aiming=false;s.activity.steady=0;}if(prevButtons.length&&s.mode==='fishing')s.activity.reel=false;prevButtons=[];return;}
 ...
 if(s.mode==='departure'&&!ui.title&&!ui.dialog){if(edge(9)||edge(1))doDeparture('depPause');if(edge(12)||edge(14))focusStep(-1);if(edge(13)||edge(15))focusStep(1);if(edge(0))document.activeElement?.click();prevButtons=b;return;}
 if(edge(9)){ui.dialog?closeModal():openPause();prevButtons=b;return;}
 if(!ui.title&&!ui.paused&&s.mode==='hunting'&&s.activity.phase==='field'){
  if(edge(1)){resetInput();dispatch({type:'huntLeave'});prevButtons=b;return;}
  const g=s.activity,trigger=b[7]||b[0],before=prevButtons[7]||prevButtons[0];
  if(Math.hypot(ax,ay)>.15)dispatch({type:'huntAim',x:g.aim.x+ax*dt*.4,y:g.aim.y+ay*dt*.3});
  if(trigger&&!before)dispatch({type:'huntAim',value:true});
  if(!trigger&&before)dispatch({type:'huntFire'});
...
window.render_game_to_text=()=>JSON.stringify({...summary(s),mode:ui.prologue?'prologue':ui.title?'intro':s.mode,prologue:prologueState(ui.prologue),departure:departureSummary(s),departurePreview,title:BRAND.title,storedMode:s.mode,overlay:ui.dialog,paused:ui.paused,coordinates:'Scene x and y are normalized 0..1 from the top left; r is a CSS-pixel hit radius.',hitTargets:ui.hits,weatherRadio:s.packed.includes('radio'),huntingPreview,controls:[...document.querySelectorAll('button:not(:disabled)')].filter(x=>x.getClientRects().length).map(x=>({action:x.dataset.action||x.id,id:x.dataset.id,label:x.innerText}))});
...
if(!huntingPreview&&(departurePreview?forceOpening:shouldAutoOpen({seen:!!openingSeen(),hasSave:!!saved||!!oldSave,reducedMotion:s.settings.reducedMotion||matchMedia('(prefers-reduced-motion: reduce)').matches,saveData:!!navigator.connection?.saveData,force:forceOpening})))beginPrologue(departurePreview?false:forceOpening);
