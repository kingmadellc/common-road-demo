import {BRAND} from './brand.js?v=0.7.2-canon-1';
import {INTRO_KEY,newPrologue,prologueState,readOpeningHistory,rememberOpening,shouldAutoOpen} from './prologue.js?v=0.7.2-canon-1';
import {OpeningPlayer} from './opening-player.js?v=0.7.2-canon-1';
import {VERSION,SAVE_KEY,FAMILY,NODES,ROADS,PACKING,PARTS,CAMPAIGN,objective,admission,node,roads} from './world.js?v=0.7.2-canon-1';
import {fresh,load,save,act,update,summary,slots,clamp} from './sim.js?v=0.7.2-canon-1';
import {render,ready,HAZARDS,HOME_POINTS,sources} from './art.js?v=0.7.2-canon-1';
import {castEndpoint} from './fishing.js?v=0.7.2-canon-1';
import {yardView} from './scenes.js?v=0.7.2-canon-1';
import {SITE_NAMES} from './salvage.js?v=0.7.2-canon-1';
import {collectorOptions,incidentOptions,threatLabel} from './conflict.js?v=0.7.2-canon-1';
import {hurt,shopStock,OFFERS} from './state.js?v=0.7.2-canon-1';
import {SHOTS} from './journey.js?v=0.7.2-canon-1';
import {enableAudio,soundFrame} from './sound.js?v=0.7.2-canon-1';
import {knowledge,ORIGIN,RADIO_MESSAGE} from './narrative.js?v=0.7.2-canon-1';
...
function panel(){
 ...
 if(s.mode==='ending')return group(s.ending.title,`<p>${s.ending.text}</p>${s.ending.owned?`<div class="deed"><div class="label">SIGNALS END CHARTER · RECORDED OWNERSHIP</div><h3>JACK & SARAH MERCER</h3><p>The lights work without a login. Jack can put down his tools without losing the house. Sarah has nobody’s agents to supervise.<br>Phones, computers and digital devices stayed at the Line. No rent. No employer controls the door. Shared dues are public, voted on and capped.</p><p>${s.packed.includes('chair')?'Grandad’s chair':'Your armchair'} sits ${s.home.spot==='window'?'beside the window':'by the workbench'}. Ben wants to learn the water wheel. Annie has a drawing to show them. This time nobody says “after the shift.”</p></div>`:''}<p>${s.stats.salvaged} items recovered · ${s.stats.fish} fish landed · ${s.stats.repairs} repairs completed · ${count(s.hour)} hours</p><div class="buttons">${button('newConfirm',undefined,'Another pilgrimage','Try the other road.','primary')}${button('journal',undefined,'Read the journey')}</div>`,true);
 return '';
}
