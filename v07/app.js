import {freshCapacity} from './hunting.js?v=0.8.0-scale-1';
import {CORPORATIONS} from './institutions.js?v=0.8.0-hunt-1';
import {BRAND} from './brand.js?v=0.8.0-hunt-1';
import {INTRO_KEY,newPrologue,prologueState,readOpeningHistory,rememberOpening,shouldAutoOpen} from './prologue.js?v=0.8.0-hunt-1';
import {OpeningPlayer} from './opening-player.js?v=0.8.0-hunt-1';
import {VERSION,SAVE_KEY,FAMILY,NODES,ROADS,PACKING,PARTS,CAMPAIGN,objective,admission,node,roads} from './world.js?v=0.8.0-hunt-1';
import {fresh,load,save,act,update,summary,slots,clamp} from './sim.js?v=0.8.0-scale-1';
import {render,ready,HAZARDS,HOME_POINTS,sources} from './art.js?v=0.8.0-scale-1';
import {castEndpoint} from './fishing.js?v=0.8.0-hunt-1';
import {yardView} from './scenes.js?v=0.8.0-hunt-1';
import {SITE_NAMES} from './salvage.js?v=0.8.0-hunt-1';
import {collectorOptions,incidentOptions,threatLabel} from './conflict.js?v=0.8.0-hunt-1';
import {hurt,shopStock,OFFERS} from './state.js?v=0.8.0-hunt-1';
import {SHOTS} from './journey.js?v=0.8.0-hunt-1';
import {enableAudio,soundFrame} from './sound.js?v=0.8.0-hunt-1';
import {knowledge,ORIGIN,RADIO_MESSAGE,arriveNarrative} from './narrative.js?v=0.8.0-hunt-1';
...
function finishNew(){if(huntingPreview){location.href='./?v=0.8.0';return;}const settings={...s.settings};resetInput();s=fresh(Math.floor(Math.random()*100000));s.settings=settings;ui.title=false;ui.dialog=null;ui.paused=false;if($('dialog').open)$('dialog').close();persist();drawUI(true);}
...
function drawUI(force=false){
 ...
 const region=huntingPreview?'HUNTING PREVIEW · SAVE UNTOUCHED':s.mode==='home'||s.ending?.owned?'SIGNALS END · THE FIRST EVENING':s.mode==='packing'?'THE MERCER HOUSEHOLD':node(s.node).region;
...
window.render_game_to_text=()=>JSON.stringify({...summary(s),mode:ui.prologue?'prologue':ui.title?'intro':s.mode,prologue:prologueState(ui.prologue),title:BRAND.title,storedMode:s.mode,overlay:ui.dialog,paused:ui.paused,coordinates:'Scene x and y are normalized 0..1 from the top left; r is a CSS-pixel hit radius.',hitTargets:ui.hits,weatherRadio:s.packed.includes('radio'),huntingPreview,controls:[...document.querySelectorAll('button:not(:disabled)')].filter(x=>x.getClientRects().length).map(x=>({action:x.dataset.action||x.id,id:x.dataset.id,label:x.innerText}))});
...
if(!huntingPreview&&shouldAutoOpen({seen:!!openingSeen(),hasSave:!!saved||!!oldSave,reducedMotion:s.settings.reducedMotion||matchMedia('(prefers-reduced-motion: reduce)').matches,saveData:!!navigator.connection?.saveData,force:forceOpening}))beginPrologue(true);
