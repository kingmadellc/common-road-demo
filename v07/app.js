import {startDeparture,departureAction,updateDeparture,departureScene,departureSummary,DECISION_INDEX,NOTICE} from './departure.js?v=0.8.1-departure-1';
import {freshCapacity} from './hunting.js?v=0.8.1-departure-1';
import {CORPORATIONS} from './institutions.js?v=0.8.1-departure-1';
import {BRAND} from './brand.js?v=0.8.1-departure-1';
import {INTRO_KEY,newPrologue,prologueState,readOpeningHistory,rememberOpening,shouldAutoOpen} from './prologue.js?v=0.8.1-departure-1';
import {OpeningPlayer} from './opening-player.js?v=0.8.1-departure-1';
import {VERSION,SAVE_KEY,FAMILY,NODES,ROADS,PACKING,PARTS,CAMPAIGN,objective,admission,node,roads} from './world.js?v=0.8.1-departure-1';
import {fresh,load,save,act,update,summary,slots,clamp} from './sim.js?v=0.8.1-departure-1';
import {render,ready,HAZARDS,HOME_POINTS,sources} from './art.js?v=0.8.1-departure-1';
import {castEndpoint} from './fishing.js?v=0.8.1-departure-1';
import {yardView} from './scenes.js?v=0.8.1-departure-1';
import {SITE_NAMES} from './salvage.js?v=0.8.1-departure-1';
import {collectorOptions,incidentOptions,threatLabel} from './conflict.js?v=0.8.1-departure-1';
import {hurt,shopStock,OFFERS} from './state.js?v=0.8.1-departure-1';
import {SHOTS} from './journey.js?v=0.8.1-departure-1';
import {enableAudio,soundFrame} from './sound.js?v=0.8.1-departure-1';
import {knowledge,ORIGIN,RADIO_MESSAGE,arriveNarrative} from './narrative.js?v=0.8.1-departure-1';
...
function drawDepartureUI(force=false){
 ...
 $('opening').innerHTML=`<section class="departure-stage ${shot.notice?'has-notice':''}" aria-label="The night we left"><div class="departure-top">${button('depPause',undefined,ui.paused?'Resume':'Pause')}${!decision?button('depSkip',undefined,shot.loading?'Skip loading':'Skip to decision'):''}</div>${shot.notice?`<aside class="departure-notice"><strong>INDEX</strong><h2>Household restricted</h2><dl><dt>Joint wallet</dt><dd>Funds frozen · Core contract flag</dd><dt>Plenty order</dt><dd>Payment declined</dd><dt>Forma staff flat</dt><dd>Return keys by 06:00 tomorrow</dd></dl></aside>`:''}<div class="departure-copy" aria-live="polite"><p class="departure-kicker">${shot.kicker}</p>${shot.speaker?`<p class="departure-speaker">${shot.speaker}</p>`:''}<p class="departure-line">${shot.line}</p></div><div class="departure-actions">${decision?button('depGo',undefined,'Bea first. Get everyone in.','','primary'):button('depNext',undefined,shot.loading?'Next cut →':'Next →','','secondary')}${decision||shot.notice?button('depNotice',undefined,'Read the notice','','secondary'):''}</div>${departurePreview?'<p class="departure-footnote">Departure preview · your journey save is untouched</p>':''}</section>`;
 panelKey=key;
