import {node} from './world.js?v=0.4.0';
import {drawRoad} from './motion.js?v=0.4.0';
import {drawSalvage,drawFishing} from './scenes.js?v=0.4.0';
...
export function render(canvas,s,ui){
 ...
 let art=ui.title?'familyPhoto':s.mode==='home'||s.ending?.owned?'home':s.mode==='crossing'?'gate':s.mode==='fishing'?'fishing':['travel','incident'].includes(s.mode)?'road':s.mode==='salvage'||s.mode==='repair'?'garage':s.mode==='packing'?'road':node(s.node).art;
 const shot=s.mode==='story'?SHOTS[s.story.id]:s.mode==='travel'&&s.road.shot?SHOTS[s.road.shot.id]:s.mode==='incident'&&s.incident.collectors?{image:'collectors'}:null;
 if(shot)art=shot.image;
