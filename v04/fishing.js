import {random} from './world.js?v=0.4.0';
const clamp=(v,a=0,b=1)=>Math.max(a,Math.min(b,v));
export const FISH_PROFILES={
 small:{name:'River fish',portions:2,stamina:8,speed:.027,run:1.5,recover:3.4},
 dart:{name:'Silver runner',portions:4,stamina:17,speed:.042,run:2.3,recover:2.8},
 heavy:{name:'Deep-water fish',portions:6,stamina:25,speed:.026,run:3.2,recover:2.3}
};
...
export function fishingAction(s,a){
 ...
 if(a.type==='cast'){
  ...
  if(a.type==='fishKeep'){
   ...
   const amount=p.portions/4;s.meals+=amount;g.catch+=p.portions;s.stats.fish++;s.freshFood.push({meals:amount,expires:s.hour+(s.packed.includes('cooler')?24:6)});s.flags.fishDinner=true;
